...
local function on_metadata_inventory_put(pos, listname, index, stack, player)
	...
	if update_formspec then
		...
		if api.formspec_data.get(player_name, 'psuedo_metadata') then
			core.show_formspec(player_name, 'robot_inventory', nodeinfo.meta():get_string('formspec'))
		end
	end
end
...
local function after_dig_node(pos, oldnode, oldmeta_table, player)
	...
	stackmeta:set_string('abilities', core.serialize(ability_table))
...
local function after_place_node(pos, player, itemstack)
	...
	if player and player:is_player() then
		...
		local player_name = player:get_player_name()
		local ability_table = ability_str ~= "" and core.deserialize(ability_str) or {}
...
local function after_construct(nodeinfo)
	...
	if old_pos_str ~= "" then
		local old_pos = core.string_to_pos(old_pos_str)
...
local function try_after_construct(pos)
	...
	if meta:get_string('player_name') == '??' then
		core.after(0, try_after_construct, pos)
	else
